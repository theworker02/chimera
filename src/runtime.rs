...
use crate::protocol::{ComputeReceipt, NodeId, TaskSlice, WasmSnapshotMeta};

/// Minimal embedded wasm: exports chimera_execute that echoes checksum of seed.
/// Built as wat-like hand bytes for demo when guest .wasm not present.
fn embedded_demo_wasm() -> &'static [u8] {
    // Precompiled minimal module is awkward; we compile a tiny wat via wasmtime if needed.
    // Instead ship rustc-built guest when available; fallback synthesizes via Module::new from wat bytes.
    ...
    pub fn execute(&self, task: &TaskSlice) -> anyhow::Result<ExecResult> {
        ...
        Ok(ExecResult {
            ...
            snapshot: WasmSnapshotMeta {
                ...
                note: "Linear memory checkpointed; Wasmtime call-stack IP is not fully portable — resume restarts guest entry with checkpoint_offset.".into(),
            },
        })
    }
