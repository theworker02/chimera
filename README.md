<p align="center">
  <img src="./assets/brand/chimera-banner.svg" alt="Chimera Ã¢â‚¬â€ decentralized peer-to-peer compute mesh" width="100%"/>
</p>
...
<p align="center">
  ...
  <a href="./LICENSE"><img src="https://img.shields.io/badge/license-Proprietary%20(source--available)-00F0FF?labelColor=0A0A0C" alt="License: MIT"/></a>
...
**Chimera** turns LAN or secure-mesh machines into a unified virtual compute cluster Ã¢â‚¬â€ no master server, no control plane. Peers gossip, steal work, execute Wasm in a sandbox, stream content-addressed data, share memory pages, and negotiate jobs through local agents.
...
| 2 | ChimeraFS Ã¢â‚¬â€ CAS, DHT, prefetch, VirtualMount |
| 3 | ChimeraMEM Ã¢â‚¬â€ DSM, Wasm migration, CRDTs, tiering |
| 4 | Agents, intents, self-healing, ed25519 receipts |
| 5 | Brand system, ADRs/RFCs, docs, CI |
| 6 | **Chimera Nano-Kernel** Ã¢â‚¬â€ `no_std` core, wasmi tier, smoltcp framing, ML-KEM/ML-DSA handshake, deterministic replay |
| 7 | Enterprise mgmt Ã¢â‚¬â€ REST portal, RBAC, audit, join tokens, chimeractl, SDKs |
| 8 | **Chimera Nexus** Ã¢â‚¬â€ frame-budget RT scheduler, distributed ECS, C ABI / WIT |
| 9 | **Nexus Core** Ã¢â‚¬â€ Wasm function gateway, DHT registry routing, Raft KV, autoscaler, `chimeractl up` |
| 10 | **WorldOS** Ã¢â‚¬â€ MeshShell SPA, Freight packages, credit ledger, TCP bridge, CRDT collab |
| 11 | **Sovereign** Ã¢â‚¬â€ chimera-usb, WebGL dash, TEE/mTLS, retro-scaling, continuity |
| 12 | **Omniverse** Ã¢â‚¬â€ 28 modular crates/packages (`crates/` + `packages/`) |
| 14 | **Boot-Sovereign** Ã¢â‚¬â€ safety-gated USB flash/recovery (`chimera-boot`) |
| 15 | **Distribution** Ã¢â‚¬â€ crates.io naming, release pipeline, mdBook, binstall/brew |
...
Docs: [ADR-0024](./docs/adr/0024-distribution-publishing.md) Ã‚Â· [RELEASING.md](./RELEASING.md)
...
## Phase 14 Ã¢â‚¬â€ Chimera Boot-Sovereign

> **Ã¢Å¡Â  DATA LOSS WARNING:** `chimera-boot` can wipe disks. Defaults keep **dry-run ON**. Physical writes require `--yes-i-understand-this-destroys-data` **and** `--no-dry-run`, and **hard-refuse** non-removable/system disks. Automated tests use **file-backed images only**. **Real-hardware flashing is UNTESTED.**
...
| `PhysicalDiskTarget` (Windows/Linux) | **implemented, gated** Ã¢â‚¬â€ **UNTESTED on hardware** |
...
## Phase 6 Ã¢â‚¬â€ Chimera Nano-Kernel (CNK)
...
| UEFI / Cortex-M / RISC-V boots | **Scaffolding only** Ã¢â‚¬â€ see [docs/guides/cnk-targets.md](./docs/guides/cnk-targets.md) |

Docs: [ADR-0006](./docs/adr/0006-chimera-nano-kernel.md) Ã‚Â· [ADR-0007](./docs/adr/0007-pq-hybrid-handshake.md) Ã‚Â· [ADR-0008](./docs/adr/0008-deterministic-replay.md) Ã‚Â· [RFC-0003](./docs/rfc/0003-cnk-pq-frames.md)

## Phase 7 Ã¢â‚¬â€ Enterprise management
...
Docs: [ADR-0009](./docs/adr/0009-observability.md) Ã‚Â· [ADR-0010](./docs/adr/0010-mgmt-rbac-sdks.md) Ã‚Â· [ADR-0011](./docs/adr/0011-deployment.md) Ã‚Â· [RFC-0004](./docs/rfc/0004-protocol-versioning.md)

## Phase 8 Ã¢â‚¬â€ Chimera Nexus (real-time interop)

Host crate [`crates/scheduler-rt/`](./crates/scheduler-rt/) (`chimera-nexus`): sub-16ms frame scheduler, ECS on TxLog, client prediction, C ABI + WIT. Engine embeds (Godot/Unreal) are **scaffolding** Ã¢â‚¬â€ unit-tested host APIs only.

```bash
cargo test -p chimera-nexus
```

## Phase 9 Ã¢â‚¬â€ Nexus Core (distributed application mesh)
...
| Service registry + failover routing | **Working** (userspace Ã¢â‚¬â€ **not** IP anycast) |
...
# 3Ã¢â‚¬â€œ4) Deploy demo add1 Wasm and invoke (input 0x29 -> 42)
./target/release/chimeractl.exe deploy demo --name add1
./target/release/chimeractl.exe invoke --function add1 --input-hex 29
```

Docs: [ADR-0012](./docs/adr/0012-function-gateway.md) Ã‚Â· [ADR-0013](./docs/adr/0013-raft-kv.md) Ã‚Â· [ADR-0014](./docs/adr/0014-service-routing.md) Ã‚Â· [RFC-0005](./docs/rfc/0005-nexus-gateway.md)

## Phase 10 Ã¢â‚¬â€ WorldOS (decentralized hypergrid environment)

Grounded P2P OS surfaces on the existing mesh Ã¢â‚¬â€ not planet-scale marketing claims.
...
Docs: [ADR-0015](./docs/adr/0015-freight.md) Ã‚Â· [ADR-0016](./docs/adr/0016-credit-economy.md) Ã‚Â· [ADR-0017](./docs/adr/0017-mesh-bridging.md) Ã‚Â· [RFC-0006](./docs/rfc/0006-worldos.md)

## Phase 11 Ã¢â‚¬â€ Chimera Sovereign
...
| Continuity / partition recovery tests | **Working** (data loss prevention via replay Ã¢â‚¬â€ not wire ZPL) |
| Driverless USB autostart | **Roadmap** |

```bash
cargo run -p usb-daemon -- --benchmark-startup --root ./data/usb-lab
# example measured on this Windows host (debug/release vary): ~100Ã¢â‚¬â€œ150ms portable init Ã¢â‚¬â€ always re-measure
cargo run --bin chimera -- --name sovereign --no-tui
# open http://127.0.0.1:7600/meshshell -> Sovereign Dash
```

Docs: [ADR-0018](./docs/adr/0018-tee-attestation.md) Ã‚Â· [ADR-0019](./docs/adr/0019-mtls-quic.md) Ã‚Â· [ADR-0020](./docs/adr/0020-retro-scaling.md) Ã‚Â· [ADR-0021](./docs/adr/0021-continuity.md) Ã‚Â· [RFC-0007](./docs/rfc/0007-sovereign.md) Ã‚Â· [ADR-0022](./docs/adr/0022-omniverse-modules.md)
...
Palette: **void** `#0A0A0C` Ã‚Â· **electric cyan** `#00F0FF` Ã‚Â· **warning amber** `#FFB800`  
Guidelines & SVG lockups: [`brand/brand.md`](./brand/brand.md) Ã‚Â· [`assets/brand/`](./assets/brand/)
...
- ADRs: [`docs/adr/`](./docs/adr/) Ã¢â‚¬â€ QUIC, Wasmtime, CAS, DSM, agents, CNK, PQ, determinism, gateway, Raft, routing, Omniverse modules
- RFCs: [`docs/rfc/`](./docs/rfc/) Ã¢â‚¬â€ wire protocol, receipts, CNK/PQ frames, Nexus gateway
- Guides: [local mesh](./docs/guides/local-mesh.md) Ã‚Â· [Wasm guests](./docs/guides/wasm-guest.md) Ã‚Â· [CNK targets](./docs/guides/cnk-targets.md) Ã‚Â· [contributing](./docs/guides/contributing.md)
